...
(function (GF) {
  ...
  // grid: rows × cols array of colour keys or null.
  // extras: { piece: [{x,y,color}], ghost: [{x,y,color}], glow: Set("x,y") }
  function drawBoard(ctx, grid, s, extras) {
    extras = extras || {};
    ...
    const at = (x, y) => (y >= 0 && y < rows && x >= 0 && x < cols ? grid[y][x] : null);

    if (extras.ghost) {
      const gAt = cellLookup(extras.ghost);
      for (const g of extras.ghost) {
        drawGhost(ctx, g.x * s, g.y * s, s, g.color, linksFor(gAt, g.x, g.y, g.color));
      }
    }

    const glow = extras.glow || new Set();
    ...
    if (extras.piece) {
      const pAt = cellLookup(extras.piece);
      for (const p of extras.piece) {
  ...
  function drawPieceCentered(ctx, cells, cx, cy, s) {
    ...
    for (const c of cells) {
      drawGel(ctx, ox + c.x * s, oy + c.y * s, s, c.color, linksFor(at, c.x, c.y, c.color));
    }
  }
